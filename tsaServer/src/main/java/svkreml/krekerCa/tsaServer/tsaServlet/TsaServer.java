...
import svkreml.krekerCa.krekerCore.KeyWrapper.CertAndKey;
import svkreml.krekerCa.krekerCore.Utils.MeUtils;
import svkreml.krekerCa.krekerCore.cryptoAlg.CryptoAlgFactory;
import svkreml.krekerCa.krekerCore.fileManagement.CertEnveloper;
import svkreml.krekerCa.krekerCore.fileManagement.FileManager;
...
public class TsaServer extends HttpServlet {
    ...
    private TimeStampTokenGenerator createTokenGenerator() {
        ...
        try {
            SignerInfoGenerator signerInfoGen = sigBuilder.build(new JcaContentSignerBuilder(CryptoAlgFactory.getInstance(tsaCertAndKey.getPrivateKey().getAlgorithm()).getSignatureAlgorithm()).setProvider("BC").build(tsaCertAndKey.getPrivateKey()), tsaCertAndKey.getCertificate());
            DigestCalculatorProvider  digestCalculatorProvider = new JcaDigestCalculatorProviderBuilder().setProvider("BC").build();
        ...
        } catch (OperatorCreationException | CertificateEncodingException | IOException | TSPException e) {
            e.printStackTrace();
